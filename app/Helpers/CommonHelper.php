            ...
            if($all_language->count() > 0){
                foreach($all_language as $language){
                    ...
                    if(DB::table('languages')->where('name', $language->name)->where('phrase', $phrase)->get()->count() == 0){
                        DB::table('languages')->insert(array('name' => $language->name, 'phrase' => $phrase, 'translated' => $translated));
                    }
                }
            }else{
                DB::table('languages')->insert(array('name' => 'english', 'phrase' => $phrase, 'translated' => $translated));
            }
            return $translated;
        }
...
if (!function_exists('get_active_currency')) {
    function get_active_currency()
    {
        ...
        if (isset($global_system_currency[0])) {
            # code...
        ...
        return $global_system_currency = 'NGN' ;
    }
}
