...
class SuperAdminController extends Controller
{
    ...
    public function createSchool(Request $request)
    {
        ...
        if (count($duplicate_school_email_check) == 0 && count($duplicate_admin_email_check) == 0) {
            ...
            if ($request->school_logo) {
                ...
                $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->school_logo->getRealPath());
                $imageUrl = $cloudinaryResponse['secure_url'];

                $school->school_logo = stripslashes($imageUrl);
                $school->save();
            }
            ...
            if (isset($school->id) && $school->id != "") {
                ...
                if (!empty($data['photo'])) {
                    ...
                    $cloudinary = new Cloudinary();
                    $cloudinaryResponse = $cloudinary->uploadApi()->upload($data['photo']->getRealPath());
                    $imageUrl = $cloudinaryResponse['secure_url'];

                    $photo  = stripslashes($imageUrl);
                } else {
                    $photo = '';
                }
    ...
    public function systemUpdate(Request $request)
    {
        ...
        if (!empty($request->email_logo)) {
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->email_logo->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];        
            GlobalSettings::where('key', 'email_logo')->update(['value' => stripslashes($imageUrl)]);
        }
        if (!empty($request->socialLogo1)) {
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->socialLogo1->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];
            
            GlobalSettings::where('key', 'socialLogo1')->update(['value' => stripslashes($imageUrl)]);
        }
        if (!empty($request->socialLogo2)) {
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->socialLogo2->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];
            
            GlobalSettings::where('key', 'socialLogo2')->update(['value' => stripslashes($imageUrl)]);
        }
        if (!empty($request->socialLogo3)) {
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->socialLogo3->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];
            
            GlobalSettings::where('key', 'socialLogo3')->update(['value' => stripslashes($imageUrl)]);
        }
        if (!empty($request->front_logo)) {
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->front_logo->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];

            GlobalSettings::where('key', 'front_logo')->update(['value' => stripslashes($imageUrl)]);
        }
        if (!empty($request->off_pay_ins_file)) {
            ...
            GlobalSettings::where('key', 'off_pay_ins_file')->update(['value' => stripslashes($imageUrl)]);
        }
    ...
    function profile_update(Request $request)
    {
        ...
        if (empty($request->photo)) {
            ...
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->photo->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];

            $user_info['photo'] = stripslashes($imageUrl);
        }
    ...
    function update_logo(Request $request)
    {
        ...
        if (!empty($request->dark_logo)) {
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->dark_logo->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];
            
            GlobalSettings::where('key', 'dark_logo')->update(['value' => stripslashes($imageUrl)]);
        }
        if (!empty($request->light_logo)) {
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->light_logo->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];

            GlobalSettings::where('key', 'light_logo')->update(['value' => stripslashes($imageUrl)]);
        }
        if (!empty($request->favicon)) {
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->favicon->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];

            GlobalSettings::where('key', 'favicon')->update(['value' => stripslashes($imageUrl)]);
        }
        if (!empty($request->white_logo)) {
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload($request->white_logo->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];

            GlobalSettings::where('key', 'white_logo')->update(['value' => stripslashes($imageUrl)]);
        }
