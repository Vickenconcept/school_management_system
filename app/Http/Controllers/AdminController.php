...
class AdminController extends Controller
{
    ...
    public function adminDashboard()
    {
        if (auth()->user()->role_id != "") {
            ...
            $active = School::where('id', auth()->user()->school_id)->where('status', 1)->first();
            ...
            if (!empty($active)) {
                ...
                return redirect()->route('login')
                    ->with('error', 'account not active, contact admin');
            }
        } else {
    ...
    public function studentDelete($id)
    {
        ...
        $fee_history = StudentFeeManager::get()->where('student_id', $id)->first();
        if ($fee_history) {
            $fee_history->delete();
        }

        $attendances = DailyAttendances::get()->where('student_id', $id)->first();
        if ($attendances) {
            $attendances->delete();
        }

        $book_issues = BookIssue::get()->where('student_id', $id)->first();
        if ($book_issues) {
            $book_issues->delete();
        }

        $gradebooks = Gradebook::get()->where('student_id', $id)->first();
        if ($gradebooks) {
            $gradebooks->delete();
        }

        $payments = Payments::get()->where('user_id', $id)->first();
        if ($payments) {
            $payments->delete();
        }

        $payment_history = PaymentHistory::get()->where('user_id', $id)->first();
        if ($payment_history) {
    ...
    public function offlineAdmissionCreate(Request $request)
    {
        ...
        if (!empty($data['photo'])) {
            ...
            $data['document_image'] = stripslashes($imageUrl);
    ...
    public function attendanceTake(Request $request)
    {
        ...
        return redirect()->back()->with('message', 'No update done');
    }
    ...
    public function sectionUpdate(Request $request, $id)
    {
        ...
        foreach ($section_id as $key => $value) {
            ...
            if (strpos($value, 'delete') == true) {
                ...
                $section = Section::find(['id' => $section_value, 'class_id' => $id])->first();
                $section->delete();
            }
        }
    ...
    public function check_payment($id)
    {
        ...
        $invoices = StudentFeeManager::where('id', $id)->first();
        return view('admin.payment_settings.check_payment', ['invoices' => $invoices]);
    }
    // 
    ...
    public function feeManagerCreate(Request $request, $value = "")
    {
        ...
        if ($value == 'single') {
            ...
            if (isset($parent_id)) {
                ...
                $data['school_id'] = auth()->user()->school_id;
                $data['session_id'] = $active_session;
                $data['payment_method'] = $data['payment_method'] ?? 'offline';
                ...
                return redirect()->back()->with('error', 'Failed. Assign Parent to Student');
            }
        } else if ($value == 'mass') {
    ...
    public function schoolUpdate(Request $request)
    {
        ...
        if ($request->school_logoo) {
            ...
            $cloudinary = new Cloudinary();
            $cloudinaryResponse = $cloudinary->uploadApi()->upload( $request->school_logoo->getRealPath());
            $imageUrl = $cloudinaryResponse['secure_url'];
            
            $school_data->school_logo =  stripslashes( $imageUrl);
            $school_data->save();
        }
        ...
        if ($request->email_logo) {
            ...
            $school_data->email_logo = stripslashes( $imageUrl);
            $school_data->save();
        }
        if ($request->socialLogo1) {
            ...
            $school_data->socialLogo1 = stripslashes( $imageUrl);
            $school_data->save();
        }
        if ($request->socialLogo2) {
            ...
            $school_data->socialLogo2 = stripslashes( $imageUrl);
            $school_data->save();
        }
        if ($request->socialLogo3) {
            ...
            $school_data->socialLogo3 = stripslashes( $imageUrl);
            $school_data->save();
        }
