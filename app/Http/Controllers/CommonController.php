...
class CommonController extends Controller
{
    ...
    public function get_student_details_by_id($id = "", $api="")
    {
        ...
        $info = json_decode(optional($student)->user_information);

        $parent_details = User::find(optional($student)->parent_id);

        $role = Role::where('role_id', optional($student)->role_id)->first();

        $class_details = Classes::find($enrol_data->class_id);

        $section_details = Section::find($enrol_data->section_id);

        $active_session = get_school_settings(optional($student)->school_id)->value('running_session');

        $school_name = School::find(optional($student)->school_id)->value('title');

        //End Fetch

        

        $enrol_data['code'] = optional($student)->code;
        $enrol_data['user_id'] = $id;
        $enrol_data['parent_name'] = $parent_details->name??"";
        $enrol_data['name'] = optional($student)->name;
        $enrol_data['email'] = optional($student)->email;
        ...
        $enrol_data['photo'] = isset($api) && $api == 'api' ?'nn': $info->photo;
        // $enrol_data['photo'] = isset($api) && $api == 'api' ? get_user_image_url($id): get_user_image($id);
        $enrol_data['school_id'] = optional($student)->school_id;
