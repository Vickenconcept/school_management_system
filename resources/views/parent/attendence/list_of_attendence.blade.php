            ...
            <div class="filter-sec">
                ...
                <form method="GET" enctype="multipart/form-data" class="d-block ajaxForm" action="{{ route('parent.daily_attendance.filter') }}">
                  <div class="att-filter d-flex flex-wrap">
                    ...
                    <div class="att-filter-option"></div>
                ...
                <p class="summary-item">{{ get_phrase('Time') }}: 
                  <span>
                    <?php if ($attendance_of_students[0]->updated_at == ""): ?>
                      {{ get_phrase('not_updated_yet') }}
                    <?php else: ?>
                      {{ date('h:i:s', strtotime($attendance_of_students[0]->updated_at)) }}
                    <?php endif; ?>
                  </span>
                </p>
              </div>
...
<script type="text/javascript">
  ...
  var download_csv=function()
  {
    ...
    if(role_id != "" && month != "" && year != ""){
      ...
      var url='{{ route("parent.dailyAttendanceFilter_csv", "month,year,role_id") }}';
