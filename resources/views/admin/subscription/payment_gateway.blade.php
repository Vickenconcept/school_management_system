    ...
    <div class="main_content paymentContent">
        <div class="paymentHeader d-flex justify-content-between align-items-center">
            ...
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
                onclick="location.href='{{ redirect()->back()->getTargetUrl() }}'"></button>
        </div>
{{-- 
        <div class="paymentWrap d-flex align-items-start flex-wrap">
            <div class="paymentLeft">
                ...
                <!-- Tab -->
                <div class="nav flex-md-column flex-row nav-pills payment_modalTab" id="v-pills-tab" role="tablist"
                    ...
                    @foreach ($active_payment_methods as $key => $value)
                        ...
                        @if ($method['status'] == 1 && addon_status('payment_gateways') == 1)
                            <div class="tabItem " id="v-pills-{{ $value['key'] }}-tab" data-bs-toggle="pill"
                                data-bs-target="#v-pills-{{ $value['key'] }}" role="tab"
                                aria-controls="v-pills-{{ $value['key'] }}" aria-selected="true">
                    ...
                    @if (addon_status('payment_gateways') != 1)
                        ...
                        <div class="tabItem " id="v-pills-offline-tab" data-bs-toggle="pill"
                            data-bs-target="#v-pills-offline" role="tab" aria-controls="v-pills-offline"
                            aria-selected="true">
                            <div class="payment_gateway_option d-flex align-items-center">
                                <div class="logo">
                                    @php
                                ...
                                <div class="info">
                                    <p class="card_no">{{ get_phrase('Offline') }}<span class="badge bg-success m-1"
                                            style="">{{ get_phrase('Addon') }}</span></p>
            ...
            <div class="paymentRight">
                ...
                <div class="tab-content payment_modalTab_content" id="v-pills-tabContent">
                    ...
                    <div class="tab-pane fade <?= $off ?>" id="v-pills-offline" role="tabpanel"
                        aria-labelledby="v-pills-offline-tab" tabindex="0">
                        <div class="off_payment_form">


                            <form
                                action="{{ route('admin.admin_subscription_offline_payment', ['id' => $selected_package['id']]) }}"
                                class="offline-form form" method="post" enctype="multipart/form-data">
                                @csrf

                                <hr class="border mb-4">

                                <input type="hidden" id="amount" class="form-control eForm-control" name="amount"
                                    value="{{ $selected_package['price'] }}" readonly>
                                ...
                                <div class="payable_document">
                                    ...
                                    <input type="file" class="form-control eForm-control-file" id="document_image"
                                        name="document_image" required>
                                </div>
        ...
        <form action="{{ route('admin.admin_subscription_offline_payment') }}"
            class="offline-form form" method="post" enctype="multipart/form-data">
            @csrf
